...
class SaleOrder(models.Model):
    ...
    @api.model_cr
    def init(self):
        self.env.cr.execute("SELECT id FROM sale_order" " WHERE reference_type IS NULL")
    ...
    def _check_communication(self):
        ...
        for order in self:
            if order.reference_type == "bba" and not invoice_obj.check_bbacomm(
                order.reference
            ):
        ...
        if self.partner_id:
            ...
            if reference_type:
                values["reference"] = invoice_obj.generate_bbacomm(
                    "out_invoice", reference_type, self.partner_id.id, ""
                )["value"]["reference"]
