...
#                     help_message)
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        # which is 'Fixed Cost'
        default_product = self.env.ref('hr_expense_v10.product_product_fixed_cost')
        pattern = '\[([^)]*)\]'
...
class HrExpenseSheet(models.Model):
    ...
    journal_id = fields.Many2one('account.journal', string='Expense Journal', states={'done': [('readonly', True)], 'post': [('readonly', True)]},
        default=lambda self: self.env['ir.model.data'].xmlid_to_object('hr_expense_v10.hr_expense_account_journal') or self.env['account.journal'].search([('type', '=', 'purchase')], limit=1),
    ...
    def _track_subtype(self, init_values):
        ...
        if 'state' in init_values and self.state == 'approve':
            return 'hr_expense_v10.mt_expense_approved'
        elif 'state' in init_values and self.state == 'submit':
            return 'hr_expense_v10.mt_expense_confirmed'
        elif 'state' in init_values and self.state == 'cancel':
            return 'hr_expense_v10.mt_expense_refused'
        elif 'state' in init_values and self.state == 'done':
            return 'hr_expense_v10.mt_expense_paid'
