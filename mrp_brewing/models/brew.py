...
class BrewDeclaration(models.Model):
    ...
    def action_confirm(self):
        brew_declaration_number = (
            self.env['ir.sequence']
                .next_by_code('brew.declaration.sequence')
        )
        self.write({
            'state': 'confirm',
...
class BrewOrder(models.Model):
        ...
        for brew_order in self:
            ...
            for child_mo in brew_order.production_order_id.child_mo_ids:
                raw_mat_moves |= (
                    child_mo
                    .move_lines2
                    .filtered(lambda record: record.state == "done")
                )
    ...
    def compute_display_name(self):
        ...
        for order in self:
            if order.state in ["done", "cancel"]:
                order.name = u'%s_%s_%s' % (order.product_id.code,
                                            year,
    ...
    brew_number = fields.Char(
        string="Brew number",
        copy=False)
    brew_beer_number = fields.Integer(
        string="Brew beer number",
        copy=False)
    state = fields.Selection(
        [('draft', 'Draft'),
         ('done', 'Done'),
    ...
    used_vessels_tank = fields.Char(
        string="Used vessels for work in tank")
    dry_extract = fields.Float(
        string="% dry extract")
    real_bulk_wort = fields.Float(
        string="Real bulk of wort")
    hl_plato_brewer = fields.Float(
        string="Hl plato noted by the brewer")
    hl_plato_agent = fields.Float(
        string="Hl plato noted by the agents")
    collecting_vessels = fields.Char(
        string="Collecting vessels")
    green_beer_volume = fields.Float(
        string="Volume of green Beer")
    sugar_quantity = fields.Float(
        string="Sugar")
    output_wort = fields.Float(
        string="Output wort")
    output_beer = fields.Float(
        string="Output beer")
    notes = fields.Char(
        string="Notes")
    ...
    def action_confirm(self):
        ...
        self.write({'state': 'done', 'brew_number': brew_year_number,
        ...
        vals = {'product_id': self.product_id.id,
                ...
                'date_planned': self.start_date, 'origin': self.name,
                'bom_id': bom_id}
