...
class MrpProduction(models.Model):
    ...
    @api.multi
    @api.depends('master_mo_id', 'master_mo_id.brew_number',
                 'brew_orders.state', 'brew_orders.brew_number',
    ...
    # fixme not used
    lot_number = fields.Many2one(
        'stock.production.lot',
        string='Lot Number')
    brew_number = fields.Char(
        string="Brew Number",
        compute=_get_brew_number)
    brew_order_name = fields.Char(
        compute=_get_brew_order_name,
        String="Brew Order Name")
    ...
    master_mo_id = fields.Many2one(
        'mrp.production',
        domain=[('product_tmpl_id.is_brewable', '=', True),
                ('state', 'not in', ['draft', 'cancel'])],
        string='Master production order')
    child_mo_ids = fields.One2many(
        'mrp.production',
        'master_mo_id',
        string="Childs production order")
