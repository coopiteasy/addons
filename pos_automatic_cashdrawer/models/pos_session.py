...
class PosSession(models.Model):
    ...
    @api.model
    def action_set_balance(self, param):
        """
        Sets the opening balance.
        Inventory is a dict with:
        { denomination: quantity }
        """
        self = self.browse(param.get('session'))
        self.check_cash_in_out_possible()
        balance = param.get('balance')
        inventory = param.get('inventory')
    ...
    @api.model
    def action_put_money_in(self, param):
        self = self.browse(param.get('session'))
    ...
    @api.model
    def action_take_money_out(self, param):
        self = self.browse(param.get('session'))
        amount = param.get('amount')
        reason = param.get('reason')
